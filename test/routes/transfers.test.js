...
it('Deve listar apenas as transferências do usuário', () => {
...
it('Deve inserir uma transferência com sucesso', () => {
...
describe('Ao salvar uma transferência válida...', () => {
  ...
  let income;
  let outcome;
  it('Deve retornar o status 201 e os dados da transferência', () => {
    return request(app).post(MAIN_ROUTE)
      .set('authorization', `Bearer ${TOKEN}`)
      .send({
        description: 'Regular Transfer', user_id: 10000, acc_ori_id: 10000, acc_dest_id: 10001, ammount: 100, date: new Date(),
...
describe('Ao tentar salvar uma transferência inválida...', () => {
  ...
  it('Não deve inserir sem descrição', () => template({ description: null }, 'Descrição é um atributo obrigatório'));
  it('Não deve inserir sem valor', () => template({ ammount: null }, 'Valor é um atributo obrigatório'));
  it('Não deve inserir sem data', () => template({ date: null }, 'Data é um atributo obrigatório'));
  it('Não deve inserir sem conta de origem', () => template({ acc_ori_id: null }, 'Conta de Origem é um atributo obrigatório'));
  it('Não deve inserir sem conta de destino', () => template({ acc_dest_id: null }, 'Conta de Destino é um atributo obrigatório'));
  it('Não deve inserir se a conta de origem e destino forem as mesmas', () => template({ acc_dest_id: 10000 }, 'Não é possível transferir de uma conta para ela mesma'));
  it('Não deve inserir se as conta pertencerem a outro usuário', () => template({ acc_ori_id: 10002 }, 'Conta #10002 não pertence ao usuário'));
});
...
describe('Ao alterar uma transferência válida...', () => {
  ...
  let income;
  let outcome;
  it('Deve retornar o status 200 e os dados da transferência', () => {
...
it('Não deve retornar transferência de outro usuário', () => {

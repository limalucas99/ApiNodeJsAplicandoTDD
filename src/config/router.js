...
module.exports = (app) => {
  ...
  app.use('/v2/users', (req, res) => res.status(200).send('V2 no ar!')); // Poderia facilmente sobrescrever qualquer rota da versão 1 utilizando essa maneira
  app.use('/v1', app.config.passport.authenticate(), protectedRouter); // Aplica a autenticação em todas as rotas
};
